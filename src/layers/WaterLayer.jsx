...
export default async function get_auckland_council_water_outages()
{
    //Get all outages
    ...
    const all_outages_with_data = await Promise.all(all_outages.map( async (item)=>({
        ...item,
  ...
  function get_water_outage_data_specific(outage_id)
{
     ...
     //Get all outages
    return fetch("https://api.watercare.co.nz/outages/outage/"+outage_id+"/details")
